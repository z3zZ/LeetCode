[how to answer leetcode](https://www.youtube.com/watch?v=OTNe0eV8418)

# How to answer a leetcode question: 
-Breakdown, take the question and find out what it is its asking
-Assign data structure and alogithm based on the constraights for the big O (vid below), if i cant find it deafult to an array
-Focus on having as little loops as possible, normally 1 loop through
-impliment the plan through python syntax by making tweaks to what is currently given
-Debug if code doesnt run as its syntax error
-Test Code if runs to make sure it works
-If code does not run then its logic error so see why its failing, maybe edge cases? 
-If not edge case walk through the code to see what each line is doing and try find it
-If you cant then add print statements to differnt parts of code to see what inputs/outputs are at that point to find where it does wrong. 
